...
class FishDBHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertFish(fish: FishVO): Boolean {
        ...
        val newRowId = db.insert(DBContract.FishEntry.TABLE_NAMEF, null, values)
    ...
    fun readAllFishes(): ArrayList<FishVO> {
        ...
        var cursor: Cursor? = null
        try {
            cursor = db.rawQuery("select * from " + DBContract.FishEntry.TABLE_NAMEF, null)
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
        }
        ...
        if (cursor!!.moveToFirst()) {
            while (cursor.isAfterLast == false) {
                ...
                fishes.add(FishVO(name_japan, price, catch_flag))
                cursor.moveToNext()
            }
        }
        return fishes
    ...
    companion object {
        ...
        private val SQL_CREATE_ENTRIES =
            "CREATE TABLE " + DBContract.FishEntry.TABLE_NAMEF + " (" +
        ...
        private val SQL_DELETE_ENTRIES = "DROP TABLE IF EXISTS " + DBContract.FishEntry.TABLE_NAMEF
    }
